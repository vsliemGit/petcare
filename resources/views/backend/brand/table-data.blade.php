...
<table class="table table-striped b-t b-light">
    ...
    @php $i++;  @endphp
    <tr>
        <td><label class="i-checks m-b-none"><input type="checkbox" class="sub_chk" data-id="{{ $brand->brand_id }}"><i></i></label></td>
        <td class="td-id">{{ $brand->brand_id }}</td>
        <td class="td-name"><span class="text-ellipsis">{{ $brand->brand_name }}</span></td>
        <td class="td-slug"><span class="text-ellipsis">{{ $brand->brand_slug }}</span></td>
        <td class="td-desc"><span class="text-ellipsis">{{ $brand->brand_desc }}</span></td>
        ...
        <td><span class="text-ellipsis">{{ $brand->brand_created_at }}</span></td>
        <td><span class="text-ellipsis">{{ $brand->brand_updated_at }}</span></td>
        <td>
        <a href="javascript:void(0)"
            class="active styling-edit edit-item" ui-toggle-class="">
            <i class="fa fa-pencil-square-o text-success text-active"></i></a>
        <a onclick="deleteItemAjax({{$brand->brand_id}})"        
...
<footer class="panel-footer">
  <div class="row">
    <div class="col-sm-5 text-center">
      <small class="text-muted inline m-t-sm m-b-sm">showing {{$listBrands->firstItem()}}-{{$listBrands->lastItem()}} of {{$listBrands->total()}} items</small>
    </div>
